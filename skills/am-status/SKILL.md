...
0. **不要自己做业务活**。接到需求的固定顺序是 `P status` → `P add` → `P dispatch`。
   哪怕活很小、哪怕自己查更快也不行 —— 你自己干就没有文件锁、没有交付记录、没有审批。
   **没 worker 上线 ≠ worker 都忙着**：前者让用户开窗口注册，后者入队等着，
   命令输出会分清楚，照它说的做。例外只有构建和 git 写，那本来就是你的活。
